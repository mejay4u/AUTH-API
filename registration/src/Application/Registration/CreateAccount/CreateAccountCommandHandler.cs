...
/// <summary>
/// Creates the portal user in the existing user table. Enforces (in order): the email must be
/// OTP-verified, the email must not already exist (duplicate check), then hashes the password with
/// the configured best-practice scheme and inserts the row with the email as the username.
/// </summary>
    ...
    : IRequestHandler<CreateAccountCommand, Result<CreateAccountResult>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var userId = await repository.CreateUserAsync(
            new NewUserRegistration(email, email, hash, salt),
            cancellationToken);
